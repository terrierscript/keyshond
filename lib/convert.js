...
const convert = (keyframeInput, options) => {
  ...
  const timings = calcTimings(kfProperties, options)
  const animationName = bindTimings(kfProperties, timings)
  const animateOptions = convertOptions(keyframes, options)
